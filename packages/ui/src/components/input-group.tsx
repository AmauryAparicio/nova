...
const inputGroupAddonVariants = cva(
  "flex h-auto min-w-0 cursor-text select-none items-center justify-center gap-2 py-1.5 font-medium text-muted-foreground text-xs group-data-[disabled=true]/input-group:opacity-50 [&>kbd]:rounded-none [&>svg:not([class*='size-'])]:size-4",
  {
    defaultVariants: {
      align: "inline-start",
    },
...
const ADDON_FOCUS_KEYS = new Set(["Enter", " "]);
...
function InputGroupAddon({
  ...
  const focusControl = useCallback((currentTarget: HTMLFieldSetElement) => {
  ...
  const handleKeyDown = useCallback(
    (e: React.KeyboardEvent<HTMLFieldSetElement>) => {
      const targetedButton = (e.target as HTMLElement).closest("button");
      if (targetedButton || !ADDON_FOCUS_KEYS.has(e.key)) {
        return;
      }
      e.preventDefault();
      focusControl(e.currentTarget);
  ...
  return (
    // biome-ignore lint/a11y/noNoninteractiveElementInteractions: must stay non-interactive (holds arbitrary children incl. real buttons) while forwarding focus to a nested control; can't be a <button>/<a> since it can contain one
    <fieldset
